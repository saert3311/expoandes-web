...
function windowScroll() {
    ...
    const logo_texts = document.getElementsByClassName("logo-text");
    if (
        document.body.scrollTop >= 50 ||
        document.documentElement.scrollTop >= 50
    ) {
        navbar.classList.add("nav-sticky");
        for (let i = 0; i < logo_texts.length; i++){
            logo_texts[i].classList.remove("text-white")
        }
    } else {
        navbar.classList.remove("nav-sticky");
        for (let i = 0; i < logo_texts.length; i++){
            logo_texts[i].classList.add("text-white")
        }
    }
}
